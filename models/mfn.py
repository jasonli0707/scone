...
class MFNWrapper(nn.Module):
    def __init__(
       ...
       mfn_configs 
        ...
        super().__init__()
        self.mfn_configs = mfn_configs
        num_layers = mfn_configs.num_layers
        dim_hidden = mfn_configs.dim_hidden
        net_type = mfn_configs.net_type
        net_configs = mfn_configs.NET
        self.dim_in = dim_in
        ...
        if net_type == 'fourier':
            input_scale=net_configs.input_scale
            weight_scale=net_configs.weight_scale
            bias=net_configs.bias
            output_act=net_configs.output_act
            self.net = FourierNet(
                        dim_in,
                        dim_hidden,
                        dim_out,
            ...
            bias=net_configs.bias
            output_act=net_configs.output_act
            alpha=net_configs.alpha
            beta=net_configs.beta
            self.net = GaborNet(
                        dim_in,
                        dim_hidden,
                        dim_out,
            ...
            raise NotImplementedError("Unknown net_type: {}".format(net_type))

    def forward(self, x, step=None, outdir=None):
        return self.net(x)